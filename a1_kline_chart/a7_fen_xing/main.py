...
def sync_excel_file(sync_path, datas):
    ...
    for row in range(1, last_row + 1):
        ...
        if len(row_values) and row_values[0] and is_integer(row_values[0]):
            # logging.info(f"规则[{k}]第[{row}]行的前四列数据：{row_values[0]},{row_values[1]},{row_values[2]},{row_values[3]}")
            if int(row_values[0]) in datas:
                sheet.range((row, 2)).value = datas[int(row_values[0])][1]
                sheet.range((row, 3)).value = datas[int(row_values[0])][2]
                sheet.range((row, 4)).value = datas[int(row_values[0])][3]
    wb.save(sync_path)
    wb.close()
    ...
    try:
        ...
        try:
            for k, v in all_files.items():
                ...
                sync_excel_file(templates[k], all_datas)
                # break   # TODO: 临时 一遍， 稍后得删除此行代码
        finally:
            app.quit()
    except Exception as e:
